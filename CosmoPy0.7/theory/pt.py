...
Current revision:
    ID:         $Id: pt.py 55 2007-10-19 05:39:22Z neyrinck $
    Date:       $Date: 2007-10-18 19:39:22 -1000 (Thu, 18 Oct 2007) $
...
def sigma2fromPk(c,r):
    ...
    """
    r = M.asarray(r)
    return 9.0/r**2*N.trapz(c.k*c.pk*sf.j1(N.outer(r,c.k))**2, M.log(c.k))/2.0/M.pi**2
...
def xi2fromCambPk(c,r):
    ...
    """
    r = M.asarray(r)
    return N.trapz(c.k**3*c.pk*sf.j0(M.outerproduct(r,c.k)),
...
def xi2fromPk(k,pk,r):
    ...
    return N.trapz(k**3*pk*sf.j0(M.outerproduct(r,k)),
                         M.log(k))/2.0/M.pi**2
...
def wPk(c,r,w):
    ...
    """
    r = M.asarray(r)
    return N.trapz(c.k**3*c.pk*w(M.outerproduct(r,c.k)),
                         M.log(c.k))/2.0/M.pi**2
...
class TriBiSpectrumFromCamb(utils.SpecialFunctions):
    ...
    def sumDotProd(self,start1,end1,start2,end2,k,cosTheta):
        ...
        #print M.outerproduct(k[start1:end1],k[start2:end2])

        ans = M.sum(M.reshape(M.outerproduct(k[start1:end1],k[start2:end2]) * \
