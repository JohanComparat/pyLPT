""">>> utils.py <<<
   ...
   ID:         $Id: utils.py 58 2008-02-23 01:39:57Z neyrinck $
   Date:       $Date: 2008-02-22 15:39:57 -1000 (Fri, 22 Feb 2008) $
...
def trapz(y, x=None, axis=-1, method=N.add.reduce):
...
def openRomb(integrand, a, b,eps=1e-6,jmax=14,k=5):
    ...
    s = N.zeros(jmaxp,N.float64)
    h = N.zeros(jmaxp+1,N.float64)
    ss = 0.
    dss = 0.
...
def interpPoly(xin,yin,n,x):
    ...
    c = N.zeros(nmax, N.float64)
    d = N.zeros(nmax, N.float64)
    ns=1
    dif=M.fabs(x-xin[1])
    for i in range(0,n):
        dift=M.fabs(x-xin[i])
...
def adaptIntPlot(f,a,b):
    ...
    while keepgoing == 1:
        ...
        newintegral = integral/2. + N.sum(eff)*dx#M.sum(f(x[:nx]+dx))*dx
...
class HGQ:
    ...
    def int(self, integrand):
        ...
        """
        return N.sum(N.outer(N.ones(integrand.shape[0],N.float64),self.weights) * \
                   integrand,1)
