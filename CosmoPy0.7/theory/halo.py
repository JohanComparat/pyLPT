...
def hodMoment(n,h):
    ...
    if h.p.hodkrav == 1:
        ...
        alpha2 = N.zeros(len(h.m),N.float64)
...
def getHaloBiasCoef(h):
    ...
    b = N.reshape(N.ones(3*len(h.sigma),N.float64),(3,len(h.sigma)))
...
def deltah(h):
    ...
    if h.p.nfw == 'y':
        # Use cisi
        ans = N.outer(h.k*0.,h.m*0.)
        cbar = N.exp(h.logcbar)
        ...
        for ki in range(len(h.k)):
            ...
            ans = N.zeros((len(h.k),len(h.m),5), N.float64)
...
def intOverMassFn(beta, mExponent, karrayunnumed, h, whichp = 0, plot = 0, show = 0,colstr = ''):
    ...
    if whichp == 'gg':
        ...
        hodNsMomentM = N.outer(hodNsMoment(mExponent,h),M.ones(5,N.float64))
        hodNsMomentMminus1 = N.outer(hodNsMoment(mExponent-1,h),M.ones(5,N.float64))
...
def getHaloTrispec(c,h, startki = 0, endki = 0, strideki = 1, adder = 0., highprecisionthresh = 0.03):
    ...
    dsq4h = 0.*h.k
    qsq = 0.*h.k
    i04 = N.outer(0.*h.k,0.*h.k)
    i12 = N.outer(0.*h.k,0.*h.k)
    i13_112 = N.outer(0.*h.k,0.*h.k)
    i13_122 = N.outer(0.*h.k,0.*h.k)
    i22 = N.outer(0.*h.k,0.*h.k)
    i114 = N.outer(0.*h.k,0.*h.k)
    i1114 = N.outer(0.*h.k,0.*h.k)
    
    
    pk1plusk3 = N.outer(0.*h.k,0.*h.k)
    t2h31 = N.outer(0.*h.k,0.*h.k)
    t3hnoB = N.outer(0.*h.k,0.*h.k)
    t3hB = N.outer(0.*h.k,0.*h.k)
    t4hnoT = N.outer(0.*h.k,0.*h.k)
    t4hT = N.outer(0.*h.k,0.*h.k)
    b10 = N.outer(0.*h.k,0.*h.k)
    t10 = N.outer(0.*h.k,0.*h.k)
    ...
    #N.savetxt(h.prefix+'dsq1h.dat',M.transpose([h.k,dsq1h]))
    #N.savetxt(h.prefix+'dsq2h.dat',M.transpose([h.k,dsq2h]))
    #N.savetxt(h.prefix+'dsq2h31.dat',M.transpose([h.k,dsq2h31]))
    #N.savetxt(h.prefix+'dsq3h.dat',M.transpose([h.k,dsq3h]))
    #N.savetxt(h.prefix+'dsq4h.dat',M.transpose([h.k,dsq4h]))
...
def getHaloCov(prefix,c,h):
    ...
    N.savetxt(prefix+'pnl.dat',N.transpose([h.k,h.pnl]), fmt = '%18.16e')

    h.prefix = prefix
    #h.dlogPnldlogA = getdlogPnldlogA(c,h)
    #N.savetxt(prefix+'dlogpnldloga.dat',M.transpose([h.k,h.dlogPnldlogA]),fmt='%6.5e')
    vk = h.k*0.
    ...
    covar += gausspart
    N.savetxt(prefix+'covar.dat',covar, fmt = '%18.16e')
    N.savetxt(prefix+'gausspart.dat',gausspart, fmt = '%18.16e')
    ...
    if h.p.outputalltterms == 1:
        N.savetxt(prefix+'pt.dat',t10, fmt = '%18.16e')
        N.savetxt(prefix+'t1h.dat',t1h, fmt = '%18.16e')
        N.savetxt(prefix+'t2h.dat',t2h, fmt = '%18.16e')
        N.savetxt(prefix+'t3h.dat',t3h, fmt = '%18.16e')
        N.savetxt(prefix+'t4h.dat',t4h, fmt = '%18.16e')
    if h.p.outputalltterms == 2:
        N.savetxt(prefix+'pt.dat',t10, fmt = '%18.16e')
        N.savetxt(prefix+'t1h.dat',t1h, fmt = '%18.16e')
        N.savetxt(prefix+'t2h22.dat',t2h22, fmt = '%18.16e')
        N.savetxt(prefix+'t2h31.dat',t2h31, fmt = '%18.16e')
        N.savetxt(prefix+'t3hB.dat',t3hB, fmt = '%18.16e')
        N.savetxt(prefix+'t3hnoB.dat',t3hnoB, fmt = '%18.16e')
        N.savetxt(prefix+'t4hT.dat',t4hT, fmt = '%18.16e')
        N.savetxt(prefix+'t4hnoT.dat',t4hnoT, fmt = '%18.16e')
    ...
    N.savetxt(prefix+'nbins.dat',M.array([len(h.k)]), fmt = '%d')
    N.savetxt(prefix+'correl.dat',correl, fmt = '%4.3f')
...
def degraderesolution(prefix,factor,dlogstring):
    ...
    for i1 in range(0,nbins-factor,factor):
        ...
        for i2 in range(0,nbins-factor,factor):
            ...
            weights2 = N.outer(k[i1:i1+factor-1]**3,k[i2:i2+factor-1]**3)
    ...
    N.savetxt(prefixnew+'pnl.dat',M.transpose([knew,pnew]), fmt = '%18.16e')
    N.savetxt(prefixnew+'covar.dat',covarnew, fmt = '%18.16e')
    N.savetxt(prefixnew+'gausspart.dat',gausspartnew, fmt = '%18.16e')
    N.savetxt(prefixnew+dlogstring,M.transpose([knew,dlognew]), fmt = '%18.16e')
    N.savetxt(prefix+'nbins.dat',M.array([nbinsnew],shape=(1,1,)), fmt = '%d')
