...
class Camb(utils.SpecialFunctions):
    ...
    def runEisensteinHu(self, sig8, getnowig=True, shiftpeak=False, shiftfact=1.):
        """
        use (much faster, but somewhat less accurate) Eisenstein & Hu
        code.
        
        use getnowig=False to get the wiggly power spectrum; 
        getnowig=True to get no-wiggles
        """
        #Output EHu file

        tilt = self.cp.scalar_spectral_index[0]
        if (shiftpeak):
            barf = self.cp.ombh2/(self.cp.ombh2+self.cp.omch2)
            f = file('ehu.cosmopy','w')
            f.write('0.25\n'+str(barf)+'\n')
            f.write('\n0.73\n0.\n')
            f.write(str(shiftfact)+'\n')
            f.write('1000.\n'+str(self.cp.transfer_k_per_logint)+'\n')
            f.close()

            # run EHu code
            os.system('ehu/shfact < ehu.cosmopy > ehu.out')
        else:
            f = file('ehu.in','w')
            
            omm = (self.cp.ombh2 + self.cp.omch2)/(self.cp.hubble/100.)**2
            f.write(str(omm)+', '+str(1.-omm)+', '+\
                        str(self.cp.omega_neutrino)+', '+str(self.cp.ombh2/(self.cp.hubble/100.)**2)+'\n')
#                        str(self.cp.omega_neutrino)+', '+str(self.cp.ombh2/self.cp.omega_(self.cp.hubble/100.)**2)+'\n')
            ...
            # run EHu code
            os.system('ehu/power < ehu.in > ehu.out')
    ...
    def gamma1(self,r,multhalfstep =(2**6+1.)/2**6):
        """ d log (sigma_r)^2/d log r """
        varplus = self.sigma2(r*multhalfstep)
        varminus = self.sigma2(r/multhalfstep)
        gamma1 = N.log(varplus/varminus)/(2.*N.log(multhalfstep))

        return gamma1

    def gamma2(self,r,multhalfstep =(2**6+1.)/2**6):
        """ d^2 log (sigma_r)^2/d log^2

        not sure where exactly to put the 2's there, but 
        it's the second logarithmic derivative of sigma^2 """


        varplus = self.sigma2(r*multhalfstep**2)
        var = self.sigma2(r)
        varminus = self.sigma2(r/multhalfstep**2)
        gamma1plus = N.log(varplus/var)/(2.*N.log(multhalfstep))
        gamma1minus = N.log(var/varminus)/(2.*N.log(multhalfstep))
        gamma2 = N.log(gamma1plus/gamma1minus)/(2.*N.log(multhalfstep))

        return gamma2

    def c21(self,cellsize,multhalfstep =(2**6+1.)/2**6):
        """ c21 xi_short xi_long = <delta^2 delta>_c """

        #radius of a sphere of the same volume as cell
        r = cellsize/(4.*N.pi/3.)**(1./3.)

        gamma1 = self.gamma1(r,multhalfstep=multhalfstep)
        return 68./21. + gamma1/3.

    def c22(self,cellsize,multhalfstep =(2**6+1.)/2**6):
        return self.c21(cellsize,multhalfstep=multhalfstep)

    def c31(self,cellsize,multhalfstep =(2**6+1.)/2**6):
        r = cellsize/(4.*N.pi/3.)**(1./3.)
        gamma1 = self.gamma1(r,multhalfstep=multhalfstep)
        gamma2 = self.gamma2(r,multhalfstep=multhalfstep)

        return 11710./441. + 61./7.*gamma1 + 2./3.*gamma1**2 + gamma2/3.

    def s3(self,cellsize,multhalfstep =(2**6+1.)/2**6):
        r = cellsize/(4.*N.pi/3.)**(1./3.)
        gamma1 = self.gamma1(r,multhalfstep=multhalfstep)
